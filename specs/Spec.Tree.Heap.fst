...
let g_tree_state (h: HS.mem) (ts: live_tree_state h): GTot (res: tree_state_wf{
  let ctx = (CB.as_seq h ts.ctx).[0] in
...
/// this invariant will not hold and partial_wf will hold after the end of the
/// iteration.
...
#push-options "--z3refresh --z3rlimit 256 --fuel 0 --ifuel 0 --z3seed 111"
let rec do_pqdownheap
  ...
  if v hole > ts1.heap_len / 2 then
    {ts1 with heap = upd ts1.heap (v hole) i}
  else
    let s = smallest ts1 root i hole in
    if s = 0ul then
      {ts1 with heap = upd ts1.heap (v hole) i}
    else
      let ts1' = {ts1 with heap = upd ts1.heap (v hole) ts1.heap.[v s]} in
...
#push-options "--z3refresh --z3rlimit 256 --fuel 1 --ifuel 1"
